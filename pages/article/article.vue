<template>
	<view>
			...
			<view class="jiangqie-page-body">
				...
				<template v-if="post.video">
				  <view class="container">
				    <video id="myVideo" :src="post.video" controls></video>
			
				  </view>
				</template>
				<mp-html v-if="post.content" :content="post.content"></mp-html>

				<view class="jiangqie-page-body-end">
					<text>- The End -</text>
				</view>
				<view class="jiangqie-page-body-tag">
					<text v-for="(item, index) in post.tags" :key="index" :data-id="item.id" :data-tag="item"
						@tap.stop="handlerTagClick">{{item}}</text>
				</view>
				...
				<template v-if="comments.length>0">
					<view v-for="(item, index) in comments" :key="index" class="jiangqie-page-cmt-content">
						<view class="jiangqie-page-cmt-avatar">
							<image :src="item.headimgurl.length>0?item.headimgurl:'/static/images/default_avatar.jpg'"
								mode="aspectFill"></image>
						</view>
						<view class="jiangqie-page-cmt-head">
							{{item.nickname}}
							<text class="jiangqie-page-cmt-time">{{item.create_time}}</text>
						...
						<!-- 	<text class="comment-action" :data-id="item.id"
							...
							<text v-if="item.is_me==1" class="comment-action" :data-id="item.id"
		...
		<view class="jiangqie-operation" v-if="!show_comment_submit && post.switch_comment==1">
			<view class="jiangqie-operation-left jiangqie-col-9 jiangqie-height-full jiangqie-ptop-zero">
				<view class="jiangqie-btn-comment" @tap.stop="handlerCommentClick">发表你的评论...</view>
			</view>
			<view class="jiangqie-operation-right jiangqie-right-flex jiangqie-col-3">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(options);
			if(options.u){
				uni.setStorageSync('jiangqie_p_id', options.u);
			}
		...
		onShow() {
			...
			var phone =  uni.getStorageSync('jiangqie_phone');
			Rest.get(Api.JIANGQIE_POST_DETAIL, {
				...
				that.comment_count = Number(res.data.comment_count);
		

				that.article = res.data.content;

		        this.miao = res.data.miao;
				...
				swan.setPageInfo({
					...
					description: that.post.excerpt,
				});
				// #endif
				
				//判断手机 没有手机跳转页面
				console.log(that.miao	)
				
				
				if(!phone && that.miao!=0){
				
		
					var miao = that.miao*1000;
		...
		onShareAppMessage() {
			...
			return {
				...
				path: 'pages/article/article?post_id=' + this.post_id+'&u='+user_id
			};
		},
		...
		methods: {
			...
			handlerLikeClick(e) {
				Rest.get(Api.JIANGQIE_USER_LIKE, {
					...
					let avatar =  uni.getStorageSync('jiangqie_head');
			...
			handlerCommentSubmit(e) {
				...
				Rest.post(Api.JIANGQIE_COMMENT_ADD, {
